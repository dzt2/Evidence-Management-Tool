...
public class DefinitionTextualAnalyzer implements TextualAnalyzer{
	/*
	 * Definition Text
	 * 		A: and(B,C,D)
	 * 		B: or(E,F)
	...
	public DefinitionTextualAnalyzer(String name){this.name=name;}
	...
	Exception getArgException(String args,String func,String reason){
		...
		code.append("Textual Analyzer "+name+" reports errors:");
	...
	public List<LogicSyntaxUnit> analysis() throws Exception {
		...
		List<LogicSyntaxUnit> list = new ArrayList<LogicSyntaxUnit>();
		for(int i=0;i<this.lines.size();i++){
			String stmt = lines.get(i);
			if(stmt==null||stmt.trim().length()==0)continue;
			
			LogicSyntaxUnit unit = this.generateUnit(stmt.trim());
			if(unit==null)
	...
	LogicSyntaxUnit generateUnit(String stmt) throws Exception{
		// Null check
		...
		if(cur>=strs.length){
			...
			return new LogicSyntaxUnit(var.trim(),type.trim(),null);
		}
		...
		return new LogicSyntaxUnit(var.trim(),type.trim(),nargs);
